#!/usr/bin/env bash
# restrinstaller - Void Linux restricted package installer
# Automates: https://docs.voidlinux.org/xbps/repositories/restricted.html#building-manually
#
# Usage:
#   restrinstaller <command> [args]
#
# Commands: search, install, update, doctor, clean, history, logs, version, help
#           selfinstall, selfuninstall
#
set -Eeuo pipefail
...
_nerd_font_supported() {
    [[ -n "${RI_USE_NERD_FONT:-}" ]] && return 0
    case "${TERM_PROGRAM:-}" in
        iTerm.app|WezTerm|Hyper|Alacritty|Kitty) return 0 ;;
    esac
    case "${TERM:-}" in
...
RI_USE_NERD_FONT=${RI_USE_NERD_FONT:-0}
if _nerd_font_supported; then
    RI_USE_NERD_FONT=1
fi

if (( RI_USE_NERD_FONT )); then
    ICON_INFO="󰋼"
    ICON_OK="󰄬"
    ICON_WARN="󰀪"
    ICON_ERROR="󰅖"
...
spinner_run() {
    ...
    shift 2
    local cmd_args=("$@")
    local tmp_out tmp_err pid spin_idx=0
    tmp_out=$(mktemp)
    tmp_err=$(mktemp)
    ...
    while kill -0 "$pid" 2>/dev/null; do
        printf "\r%s %s" "${SPINNER_CHARS[spin_idx]}" "$msg"
    ...
    # Append captured output to the main log (and package log if provided)
    if [[ -n "${RI_LOG_FILE:-}" ]]; then
        cat "$tmp_out" >> "$RI_LOG_FILE" 2>/dev/null || true
        cat "$tmp_err" >> "$RI_LOG_FILE" 2>/dev/null || true
    fi
    if [[ -n "${pkg_log:-}" ]]; then
        cat "$tmp_out" >> "$pkg_log" 2>/dev/null || true
        cat "$tmp_err" >> "$pkg_log" 2>/dev/null || true
    fi
...
log_show() {
    local filter=${1:-}
    if [[ -z "$filter" ]]; then
        if [[ -f "$RI_LOG_FILE" ]]; then "${PAGER:-cat}" "$RI_LOG_FILE"
        else ui_warn "No logs yet."; fi
        return 0
    fi
    local latest
    latest=$(ls -1t "$RI_LOG_DIR"/"$filter"-*.log 2>/dev/null | head -n1 || true)
    if [[ -z "$latest" ]]; then ui_warn "No logs found for '$filter'."; return 1; fi
...
dep_hint() {
    case "$1" in
        git|curl|jq|fzf) echo "sudo xbps-install -S $1" ;;
        sudo)            echo "sudo xbps-install -S sudo  (or use doas)" ;;
        xbps-install|xbps-query)
                         echo "Base Void package (xbps). Missing means broken system." ;;
        xbps-src)        echo "Provided by void-packages: 'restrinstaller update'" ;;
...
deps_check() {
    ...
    if (( ${#missing[@]} > 0 )); then
        ...
        for d in "${missing[@]}"; do printf '  - %s: %s\n' "$d" "$(dep_hint "$d")"; done
        return 1
    fi
    return 0
}
...
deps_check_optional() {
    ...
    for d in "${OPTIONAL_DEPS[@]}"; do
        if dep_have "$d"; then ui_ok "optional: $d present"
        else ui_warn "optional: $d missing ($(dep_hint "$d"))"; fi
    done
}

priv_cmd() {
    if [[ $EUID -eq 0 ]]; then echo ""; return 0; fi
    if dep_have sudo; then echo "sudo"; return 0; fi
    if dep_have doas; then echo "doas"; return 0; fi
    return 1
}
...
# CACHE
############################################
cache_init() { mkdir -p "$RI_CACHE_DIR" "$RI_LOG_DIR" "$RI_GH_CACHE_DIR" "$RI_CONFIG_DIR"; }

cache_age() {
    local f=$1
    [[ -f "$f" ]] || { echo 999999999; return; }
    local now mtime; now=$(date +%s)
    mtime=$(stat -c %Y "$f" 2>/dev/null || stat -f %m "$f" 2>/dev/null || echo 0)
    echo $((now - mtime))
}

cache_is_fresh() {
    local age; age=$(cache_age "$1"); (( age < ${2:-$RI_INDEX_TTL} ))
}

cache_update() {
    ui_section "Updating package index"
    if spinner_run "Fetching package index" github_fetch_index; then
        ui_ok "Package index updated ($(wc -l <"$RI_INDEX_TXT" | tr -d ' ') packages)"
    else
        ui_error "Failed to update package index"
        return 1
    fi
}
cache_ensure_index() {
    if [[ ! -f "$RI_INDEX_TXT" ]] || ! cache_is_fresh "$RI_INDEX_TXT" || [[ ! -f "$RI_CACHE_DIR/index_commit.txt" ]]; then
        cache_update
    fi
}
...
gh_get() {
    local url=$1 out=${2:-} tries=3 i http
    for ((i=1; i<=tries; i++)); do
        if [[ -n "$out" ]]; then
            ...
            http=$(curl -sSL --fail-with-body -o "$tmp" -w '%{http_code}' \
                "${_gh_headers[@]}" "$url" || true)
            if [[ "$http" =~ ^2 ]]; then cat "$tmp"; rm -f "$tmp"; return 0; fi
            rm -f "$tmp"
        fi
...
github_fetch_index() {
    ...
    [[ -z "$commit_sha" ]] && { log_error "no branch sha"; return 1; }
    ...
    [[ -z "$srcpkgs_sha" ]] && { log_error "srcpkgs missing"; return 1; }
...
github_fetch_template() {
    local pkg=$1 out="$RI_GH_CACHE_DIR/${pkg}.template"
    if cache_is_fresh "$out" 3600; then printf '%s' "$out"; return 0; fi
    ...
    if curl -sSL --fail-with-body -H "User-Agent: $RI_UA" -o "$out.tmp" "$url"; then
        mv -f "$out.tmp" "$out"; printf '%s' "$out"; return 0
    fi
...
pkg_template_path() {
    local pkg=$1 tpl="$RI_VOID_DIR/srcpkgs/$pkg/template"
    if [[ -f "$tpl" ]]; then printf '%s' "$tpl"; return 0; fi
    if tpl=$(github_fetch_template "$pkg"); then printf '%s' "$tpl"; return 0; fi
    return 1
}

pkg_is_restricted() {
    local tpl; tpl=$(pkg_template_path "$1") || return 2
...
# VOID-PACKAGES TREE
############################################
void_have_tree() { [[ -d "$RI_VOID_DIR/.git" && -x "$RI_VOID_DIR/xbps-src" ]]; }

void_clone_tree() {
    ui_info "Cloning void-packages (shallow) into $RI_VOID_DIR"
    mkdir -p "$(dirname "$RI_VOID_DIR")"
    if [[ -e "$RI_VOID_DIR" && ! -d "$RI_VOID_DIR/.git" ]]; then rm -rf "$RI_VOID_DIR"; fi
    git clone --depth=1 --filter=blob:none --branch "$RI_VOID_BRANCH" \
        "$RI_VOID_REPO_URL" "$RI_VOID_DIR" 2>&1 | tee -a "$RI_LOG_FILE"
    [[ -x "$RI_VOID_DIR/xbps-src" ]] || { log_error "xbps-src missing after clone"; return 1; }
}
void_update_tree() {
    local target_sha=${1:-}
    if ! void_have_tree; then void_clone_tree; return; fi
    ui_info "Updating void-packages"
    if spinner_run "Fetching void-packages updates" _void_update_tree_inner "$target_sha"; then
...
_void_update_tree_inner() {
    ...
    ) 2>&1 | tee -a "$RI_LOG_FILE" >/dev/null
}

void_ensure_tree() { void_have_tree || void_clone_tree; }
...
void_ensure_bootstrap() {
    ...
    if spinner_run "Bootstrapping build environment" _void_bootstrap_inner; then
...
_void_bootstrap_inner() {
    ( cd "$RI_VOID_DIR" && ./xbps-src binary-bootstrap ) 2>&1 | tee -a "$RI_LOG_FILE" >/dev/null
}

void_enable_restricted() {
    void_ensure_tree
    local etc="$RI_VOID_DIR/etc" cfg="$RI_VOID_DIR/etc/conf"
    mkdir -p "$etc"
...
builder_build() {
    ...
    (
        cd "$RI_VOID_DIR"
        if [[ -n "$RI_ARCH" ]]; then ./xbps-src -a "$RI_ARCH" pkg "$pkg"
        else ./xbps-src pkg "$pkg"; fi
    ) 2>&1
}

builder_find_binpkgs() {
    local pkg=$1 binroot="$RI_VOID_DIR/hostdir/binpkgs"
    [[ -d "$binroot" ]] || return 1
...
builder_clean() {
    ...
    if [[ ! -d "$RI_VOID_DIR" ]]; then ui_warn "No void-packages tree"; return 0; fi
    ( cd "$RI_VOID_DIR" && ./xbps-src -C zap || true ) 2>&1 | tee -a "$RI_LOG_FILE" >/dev/null || true
...
history_show() {
    if [[ ! -s "$RI_HISTORY_FILE" ]]; then ui_info "No history yet."; return 0; fi
    printf '%-20s  %-16s  %s\n' "DATE" "STATUS" "PACKAGE"
    printf '%-20s  %-16s  %s\n' "----" "------" "-------"
...
search_cmd() {
    ...
    if [[ -z "$query" ]]; then
        if dep_have fzf; then
            local sel; sel=$(fzf --prompt="package> " --height=80% --border < "$RI_INDEX_TXT" || true)
            [[ -z "$sel" ]] && { ui_info "No selection."; return 0; }
            printf '%s\n' "$sel"
    ...
    local matches; matches=$(grep -Fi -- "$query" "$RI_INDEX_TXT" || true)
    if [[ -z "$matches" ]]; then ui_warn "No matches for: $query"; return 1; fi
    if dep_have fzf; then
        local sel
...
doctor_run() {
    ...
    if [[ -r /etc/os-release ]]; then
        ...
        grep -qi 'void' /etc/os-release || { ui_warn "Does not look like Void Linux"; rc=1; }
    else
    ...
    local d
    for d in "${REQUIRED_DEPS[@]}" "${BUILD_DEPS[@]}"; do
        if dep_have "$d"; then ui_ok "$d ($(command -v "$d"))"
        else ui_error "$d MISSING — $(dep_hint "$d")"; rc=1; fi
    done
    ...
    if [[ $EUID -eq 0 ]]; then ui_ok "running as root"
    elif p=$(priv_cmd) && [[ -n "$p" ]]; then ui_ok "using: $p"
    else ui_error "no sudo/doas and not root"; rc=1; fi
    ...
    if void_have_tree; then ui_ok "present at $RI_VOID_DIR"
    else ui_warn "not cloned yet — run: restrinstaller update"; fi

    ui_info "Network"
    if curl -sSfI --max-time 5 "$RI_GH_API" >/dev/null 2>&1; then ui_ok "GitHub API reachable"
    else ui_warn "GitHub API unreachable"; fi

    if (( rc == 0 )); then ui_section "All checks passed."
    else ui_section "Some checks failed."; fi
    return $rc
}
...
installer_run() {
    local pkg=$1
    [[ -z "$pkg" ]] && { ui_error "No package specified."; return 2; }

    local pkg_log; pkg_log=$(log_pkg_file "$pkg"); : >"$pkg_log"
    log_info "Install requested: $pkg (log: $pkg_log)"

    ui_section "Installing '$pkg'"

    ui_step 1 5 "Checking dependencies"
    deps_check || { history_record "$pkg" "failed:deps"; return 1; }
    if ! priv_cmd >/dev/null; then
        ui_error "Need root or sudo/doas."
        history_record "$pkg" "failed:noprivs"; return 1
    fi
    ui_ok "Dependencies present"

    ui_step 2 5 "Preparing void-packages"
    void_ensure_tree
    ...
    if [[ ! -d "$RI_VOID_DIR/srcpkgs/$pkg" ]]; then
        ...
        if ! pkg_exists "$pkg"; then
            ...
            history_record "$pkg" "failed:notfound"; return 1
        fi
        local index_sha=""
        ...
        ui_info "Contents of srcpkgs (first 20):"
        ls -1 "$RI_VOID_DIR/srcpkgs" | head -20 | while read -r d; do ui_dim "  $d"; done
        if [[ ! -d "$RI_VOID_DIR/srcpkgs/$pkg" ]]; then
    ...
    if ! spinner_run "Installing $pkg" _install_xbps "$pkg" "$repo_dir" "$SUDO"; then
        ui_error "Install failed. See $pkg_log"
        history_record "$pkg" "failed:install"; return 1
    fi
    ui_ok "Installed"

    ui_step 5 5 "Cleaning"
    ui_ok "Done"
...
_install_xbps() {
    local pkg=$1 repo_dir=$2 SUDO=$3
    if ! $SUDO xbps-install -y -R "$repo_dir" -S "$pkg" 2>&1; then
        ui_warn "Repo install failed, retrying with --repository."
        $SUDO xbps-install -y --repository="$repo_dir" "$pkg" 2>&1
    fi
}
...
main() {
    ...
    case "$cmd" in
        search)               search_cmd "${1:-}" ;;
        install|i)
            [[ $# -ge 1 ]] || { ui_error "'install' needs a package name"; usage; exit 2; }
            installer_run "$1" ;;
        update|u)             cache_update; void_update_tree ;;
        doctor)               doctor_run ;;
        clean)                builder_clean ;;
        history)              history_show ;;
        logs)                 log_show "${1:-}" ;;
        selfinstall)          self_install ;;
        selfuninstall)        self_uninstall ;;
        version|--version|-v) echo "restrinstaller $RESTRINSTALLER_VERSION" ;;
        help|--help|-h|"")    usage ;;
        *)                    ui_error "Unknown command: $cmd"; usage; exit 2 ;;
    esac
}
